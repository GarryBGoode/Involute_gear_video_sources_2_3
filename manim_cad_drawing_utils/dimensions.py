...
class Pointer_Label_Free(VDict):
    def __init__(self,point, text:str, offset_vector=(RIGHT+DOWN),pointer_offset=0,**kwargs):
        text_buff = 0.1
...
class Linear_Dimension(VDict):
    def __init__(self,
        ...
        norm_vect_dim = normalize(diff_vect)
        ...
        if not outside_arrow:
            ...
            main_line = Line(start=startpoint + self.arrow_offset*norm_vect_dim,
                             end=endpoint - self.arrow_offset*norm_vect_dim,
                             **kwargs)
        ...
        if not outside_arrow:
            dist = np.linalg.norm(self.start - self.end)-tip_len*2
        else:
            dist = np.linalg.norm(self.start - self.end)


        if text_w > (dist*0.8):
            textmob.scale((dist*0.8)/text_w)
            self.text_h = textmob.height
