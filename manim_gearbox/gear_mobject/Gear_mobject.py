...
class Gear(VMobject):
    ...
    def generate_points(self):
        ...
        self.hmax=involute_height_func(t_hmax[0],self.rb)

        undercut = False
        if self.ra > self.rb+self.hmax:
            self.ra = self.rb+self.hmax
        ...
        if self.z < 2 / (np.sin(self.alpha * DEGREES) ** 2) or self.rf < self.rb:
            ...
            # if 'tmax_ucut' in self.gen_params.keys() and 'tmin_invo' in self.gen_params.keys():
            #     tres_ucut = fsolve(diff_val_func, np.array([self.gen_params['tmax_ucut'], self.gen_params['tmin_invo']]))
            # else:
            tres_ucut = fsolve(diff_val_func,np.array([0.4,0.05]))
            tmin = tres_ucut[1]
            ...
            # save these results maybe other animations need it
            self.gen_params['tmin_ucut'] = tmin_ucut
            self.gen_params['tmax_ucut'] = tmax_ucut
            self.gen_params['rad_offs'] = rad_ucut
            self.gen_params['tan_offs'] = tan_ucut

        else:
            self.gen_params['tmin_ucut'] = 0
            self.gen_params['tmax_ucut'] = 0
            self.gen_params['rad_offs'] = 0
            self.gen_params['tan_offs'] = 0

        trange_invo = np.linspace(-tmax,-tmin,self.nppc)
        self.gen_params['tmax_invo'] = tmax
        self.gen_params['tmin_invo'] = tmin
        ...
        if undercut:
            ...
            if not self.smoothing:
                involute_curve.append_points((self.nppc-1)*4*[involute_curve.points[-1,:]])
        ...
        for k in range(int(self.z)-self.z_cut):
    ...
    def mesh_param_calc(self, gear2: 'Gear', offset: float = 0, bias=1):
        ...
        ret =  {'diff_vect':diff_vect,'pitch_dist':pitch_dist,'mod1':mod1,'mod2':mod2,'distance':distance,
                'pitch_dist':pitch_dist, 'rp1':rp1,'rp2':rp2,'invo_offset_1':invo_offset_1,'invo_offset_2':invo_offset_2,
                'angle_offset_1':angle_offset_1,'angle_offset_2':angle_offset_2}
        return ret
    ...
    def mesh_to(self, gear2: 'Gear', offset: float = 0, bias = 1):
        ...
        mesh_param = self.mesh_param_calc(gear2,offset,bias)
        self.shift(mesh_param['diff_vect'] * (-mesh_param['distance'] + mesh_param['pitch_dist']))


        # with inside gears, the tooth goes into a tooth-hole, they overlap
        if self.inner_teeth or gear2.inner_teeth:
            self.rotate((mesh_param['mod2'] - mesh_param['mod1']) * self.pitch_angle)
        # with outside gears, the tooth pattern needs to shift half-cycle, and the rotation is reversed
        else:
            self.rotate((-mesh_param['mod2'] - mesh_param['mod1'] + 0.5) * self.pitch_angle)
...
class Rack(VMobject):
    def __init__(self, num_of_teeth, module=0.2, alpha=20, h_a=1, h_f=1.17, make_closed=True, **kwargs):
        '''
